      ...
      {items.map((item) =>
        isDropdown(item) ? (
          <div key={item.id} className="relative group">
            ...
            <div className="absolute left-0 top-full pt-2 hidden group-hover:block" style={{ zIndex: PAGE_Z.overlay }}>
...
export function MobileMenu({ items, theme }: { items: NavItem[]; theme: SiteTheme }) {
  ...
  return (
    <div className="md:hidden">
      ...
      {open && (
        <div className="fixed inset-0" style={{ zIndex: PAGE_Z.scrim, background: tint(theme.text, 0.4) }} onClick={() => setOpen(false)}>
