...
export default function TeacherTable({
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      updateSelectedIds(new Set(teachers.map(t => t.id)));
    } else {
      updateSelectedIds(new Set());
    }
  };

  const handleSelectRow = (teacherId: string, e: React.ChangeEvent<HTMLInputElement>) => {
    e.stopPropagation();
    const newSelectedIds = new Set(selectedIds);
    if (e.target.checked) {
      newSelectedIds.add(teacherId);
    } else {
      newSelectedIds.delete(teacherId);
    }
    updateSelectedIds(newSelectedIds);
  };

  const handleMenuToggle = (teacherId: string, e: React.MouseEvent) => {
    e.stopPropagation();

    if (openMenuTeacherId === teacherId) {
      setOpenMenuTeacherId(null);
      return;
    }
    ...
    setOpenMenuTeacherId(teacherId);
  };

  const handleMenuItemClick = (action: string, teacher: Teacher) => {
    if (action === 'Delete') {
      setTeacherToDelete(teacher);
      setIsDeleteModalOpen(true);
      setOpenMenuTeacherId(null);
    } else if (action === 'Edit') {
      router.push(`/teachers/edit/${teacher.id}`);
      setOpenMenuTeacherId(null);
    } else if (action === 'View Teacher') {
      router.push(`/teachers/${teacher.id}`);
      setOpenMenuTeacherId(null);
    } else {
      console.log(`${action} clicked for teacher:`, teacher.id);
      setOpenMenuTeacherId(null);
    }
  };
  ...
  const handleConfirmDelete = () => {
    if (teacherToDelete) {
      console.log('Deleting teacher:', teacherToDelete.id);
  ...
  const isSomeSelected = selectedIds.size > 0 && selectedIds.size < teachers.length;
  ...
  const allColumns: ColumnConfig<Teacher>[] = [
      ...
      render: (teacher) => (
        <div className="flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
          <input
            ...
            onChange={(e) => handleSelectRow(teacher.id, e)}
        ...
        <div className="flex items-center gap-1.5 md:gap-2.5 min-w-0">
          {teacher.imageUrl ? (
            <div
              ...
              onClick={(e) => {
                e.stopPropagation();
                console.log("Avatar clicked", teacher.id);
              }}
            >
              <img
          ...
          <Tooltip content={`${teacher.firstName} ${teacher.lastName}`}>
    ...
    },
    {
      ...
      className: "text-left w-[10%]",
      render: (teacher) => (
        <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-bold border ${getRoleBadgeColor(teacher.role)}`}>
          {teacher.role}
        </span>
      ),
    },
    {
      ...
      render: (teacher) => (
        <div className="flex items-center justify-start">
          <span
            className={`inline-flex items-center justify-center px-2 md:px-3 xl:px-3.5 py-1 md:py-1.5 xl:py-2 rounded-full text-[0.625rem] md:text-xs xl:text-sm font-semibold shadow-sm transition-all duration-300 whitespace-nowrap ${getStatusBadgeColor(teacher.employmentStatus)}`}
          >
        ...
        <div className="flex items-center justify-start gap-0.5 md:gap-1 lg:gap-1.5 xl:gap-2 pr-0.5 md:pr-2">
          <div className="relative group/msg flex-shrink-0">
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                console.log("Message", teacher.id);
              }}
            >
          ...
          <div className="relative flex-shrink-0 overflow-visible group/more" ref={openMenuTeacherId === teacher.id ? menuRef : null}>
            <button
              ref={openMenuTeacherId === teacher.id ? buttonRef : null}
              ...
              title="More"
              onClick={(e) => handleMenuToggle(teacher.id, e)}
            >
            ...
            {openMenuTeacherId === teacher.id && (
              ...
              >
                <button
                  onClick={() => handleMenuItemClick('View Teacher', teacher)}
                ...
                <button
                  onClick={() => handleMenuItemClick('Edit', teacher)}
                  className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 midnight:text-cyan-100 purple:text-pink-100 hover:bg-gray-50 dark:hover:bg-[#22262e] midnight:hover:bg-cyan-500/10 purple:hover:bg-pink-500/10 transition-colors cursor-pointer"
                >
                ...
                <button
                  onClick={() => handleMenuItemClick('Login Details', teacher)}
                  className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 midnight:text-cyan-100 purple:text-pink-100 hover:bg-gray-50 dark:hover:bg-[#22262e] midnight:hover:bg-cyan-500/10 purple:hover:bg-pink-500/10 transition-colors cursor-pointer"
                >
                ...
                <button
                  onClick={() => handleMenuItemClick('Disable', teacher)}
                  className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 midnight:text-cyan-100 purple:text-pink-100 hover:bg-gray-50 dark:hover:bg-[#22262e] midnight:hover:bg-cyan-500/10 purple:hover:bg-pink-500/10 transition-colors cursor-pointer"
                >
                  <Lock className="w-4 h-4" />
                  <span>Disable</span>
                ...
                <button
                  onClick={() => handleMenuItemClick('Delete', teacher)}
    ...
    <>
      ...
      {teacherToDelete && (
        <DeleteConfirmationModal
          ...
          itemId={teacherToDelete.staffId}
          warningMessage="This will permanently remove this teacher and all associated data. This action cannot be undone."
