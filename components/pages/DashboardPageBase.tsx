...
export interface DashboardPageBaseProps<T = unknown> {
  /** Page title */
  title: string;
  /** Optional subtitle shown below title */
  subtitle?: string;
  /** Optional description shown below title (alias of subtitle in PageHeader) */
  description?: string;
  ...
  /** Loading text shown in full-page loader */
  loadingText?: string;
}
...
export default function DashboardPageBase<T = unknown>({
  ...
  const renderStatCard = (stat: StatCardConfig<T>, index: number) => {
    ...
    const subtitleText = stat.getSubtitle?.(data);
