...
export default function CustomDropdown({
  ...
  const getVariantStyles = () => {
    if (variant === "blue") {
      return {
        ...
        selected: "bg-gray-100 dark:bg-[#22262e] midnight:bg-gray-700 purple:bg-gray-700 font-bold",
      };
    }
    return {
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      {/* Chevron Icon — snaps with dropdown open/close */}
      <ChevronRight className={`absolute right-1.5 sm:right-3 top-1/2 -translate-y-1/2 w-3 h-3 sm:w-4 sm:h-4 pointer-events-none transition-transform duration-[120ms] ease-out ${
        isOpen
          ? `${dropup ? "rotate-90" : "-rotate-90"} text-blue-600 dark:text-blue-400 midnight:text-cyan-400 purple:text-pink-400`
          : `${dropup ? "-rotate-90" : "rotate-90"} text-gray-500 dark:text-gray-400 midnight:text-cyan-400 purple:text-pink-400`
      }`} />
