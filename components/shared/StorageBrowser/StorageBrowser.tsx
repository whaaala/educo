...
export default function StorageBrowser({
  ...
  return (
    <div
      className={`flex flex-col bg-white dark:bg-gray-800 midnight:bg-gray-900 purple:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 midnight:border-cyan-500/20 purple:border-pink-500/20 overflow-hidden ${className}`}
    ...
    >
      ...
      {showToolbar && (
        <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
          ...
          <button
            ...
            className="p-1.5 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-30 transition-colors cursor-pointer"
            title="Go up"
          >
          ...
          {/* Breadcrumbs */}
          <div className="flex items-center gap-1 flex-1 min-w-0 text-xs text-gray-500 dark:text-gray-400 overflow-x-auto">
            <button
              onClick={() => setCurrentPath("")}
              className="hover:text-gray-700 dark:hover:text-gray-200 flex-shrink-0 cursor-pointer"
            >
              Root
            </button>
          ...
          <div className="relative">
            <input
              ...
              className="w-32 sm:w-40 px-2.5 py-1 pl-7 text-xs border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
            />
          ...
          <div className="flex border border-gray-300 dark:border-gray-600 rounded-lg overflow-hidden">
            <button
              ...
              className={`p-1.5 transition-colors cursor-pointer ${viewMode === "list" ? "bg-gray-200 dark:bg-gray-600" : "hover:bg-gray-100 dark:hover:bg-gray-700"}`}
            >
              <List className="w-3.5 h-3.5 text-gray-600 dark:text-gray-400" />
            </button>
            <button
              onClick={() => setViewMode("grid")}
              className={`p-1.5 transition-colors cursor-pointer ${viewMode === "grid" ? "bg-gray-200 dark:bg-gray-600" : "hover:bg-gray-100 dark:hover:bg-gray-700"}`}
            >
      ...
      {/* New folder input */}
      {showNewFolder && (
        <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-200 dark:border-gray-700 bg-blue-50 dark:bg-blue-500/10">
          ...
          />
          <button onClick={handleCreateFolder} className="text-xs text-blue-600 dark:text-blue-400 font-medium cursor-pointer">Create</button>
      ...
      <div className="flex-1 overflow-auto min-h-[200px] max-h-[400px] relative">
        ...
        {error && !isLoading && (
          <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
            ...
            <button
              onClick={() => loadFolder(currentPath)}
              className="mt-2 text-xs text-blue-600 dark:text-blue-400 cursor-pointer"
            >
              Retry
            </button>
          </div>
        ...
        {!isLoading && !error && items.length > 0 && viewMode === "list" && (
          <div className="divide-y divide-gray-100 dark:divide-gray-700/50">
            {items.map((item) => (
              <div
                key={item.id}
                className="flex items-center gap-3 px-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer group"
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex items-center gap-2">
                  ...
                  {isDropboxItem(item) && (
                    <span className="text-[9px] px-1.5 py-0.5 rounded bg-blue-100 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 flex-shrink-0">
                      Dropbox
                    </span>
                  )}
                </div>
                ...
                <span className="text-xs text-gray-400 dark:text-gray-500 hidden sm:block w-20 text-right">
                ...
                {item.type === "file" && mode === "browse" && (
                  <button
                    ...
                    className="p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-600 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                  >
        ...
        {!isLoading && !error && items.length > 0 && viewMode === "grid" && (
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2 p-3">
            {items.map((item) => (
              <div
                key={item.id}
                className="flex flex-col items-center gap-1 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer group relative"
                onClick={() => handleItemClick(item)}
              >
                <div className="w-10 h-10 flex items-center justify-center">
                  {getFileIcon(item.name, item.type)}
                </div>
                <p className="text-[11px] text-gray-700 dark:text-gray-300 text-center truncate w-full">
                  {item.name}
                </p>
                {/* Dropbox badge in grid */}
                {isDropboxItem(item) && (
                  <span className="text-[8px] px-1 py-0.5 rounded bg-blue-100 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400">
                    Dropbox
                  </span>
                )}
              </div>
        ...
        {contextMenu && (
          ...
          >
            <button
              ...
              className="flex items-center gap-2 w-full px-3 py-1.5 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 w-full px-3 py-1.5 text-xs text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 cursor-pointer"
            >
