...
export default function ConvertToVideoDialog({
  ...
  if (isComplete) {
    return (
      <div className="fixed inset-0 z-[9000] flex items-center justify-center">
        ...
        <div className="relative z-10 w-[440px] max-w-[92vw] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-8">
          <div className="text-center">
            ...
            <h3 className="text-[18px] font-semibold text-gray-900 dark:text-gray-100">Video ready!</h3>
            ...
            <div className="flex flex-col gap-2 mt-6">
              ...
              <div className="flex gap-2">
                <button onClick={() => { window.open(`mailto:?subject=${encodeURIComponent(`Video: ${title}`)}&body=${encodeURIComponent(`The video for "${title}" is ready.`)}`, "_self"); }}
                  className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl text-[12px] font-medium text-gray-600 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors cursor-pointer">
  ...
  return (
    <div className="fixed inset-0 z-[9000] flex items-center justify-center">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={onClose} />
      <div className="relative z-10 w-[520px] max-w-[92vw] max-h-[85vh] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl shadow-black/20 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 dark:border-gray-800">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-[15px] font-semibold text-gray-900 dark:text-gray-100">Convert to video</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
                ...
                <button key={opt.value} onClick={() => setScope(opt.value)}
                  className={`flex flex-col items-center gap-1.5 px-3 py-3 rounded-xl text-center transition-all duration-200 cursor-pointer ${
                    scope === opt.value
                      ...
                      : "bg-gray-50 dark:bg-gray-800/30 border-2 border-gray-100 dark:border-gray-800 hover:border-gray-200"
                  }`}>
                  <opt.icon className={`w-5 h-5 ${scope === opt.value ? "text-purple-500" : "text-gray-400"}`} />
                  <span className={`text-[12px] font-semibold ${scope === opt.value ? "text-purple-700 dark:text-purple-300" : "text-gray-600 dark:text-gray-400"}`}>{opt.label}</span>
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1">
                ...
                <input type="number" min={1} max={totalSlides} value={rangeStart} onChange={e => setRangeStart(Math.max(1, parseInt(e.target.value) || 1))}
                  className="w-full px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 text-[13px] text-center outline-none focus:border-purple-400" />
              </div>
                ...
                return (
                  <button key={slide.id} onClick={() => toggleSlide(slide.id)}
                    className={`relative rounded-lg transition-all duration-200 cursor-pointer ${
                      on ? "ring-2 ring-purple-500 ring-offset-2 shadow-md" : "ring-1 ring-gray-200 dark:ring-gray-700 opacity-50 hover:opacity-100 hover:shadow-sm"
                    }`}>
                ...
                <button key={q.value} onClick={() => setQuality(q.value)}
                  className={`flex-1 px-3 py-2.5 rounded-xl text-center transition-all cursor-pointer ${
                    quality === q.value
                      ...
                      : "bg-gray-50 dark:bg-gray-800/30 border-2 border-gray-100 dark:border-gray-800 text-gray-600 hover:border-gray-200"
                  }`}>
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[13px] font-semibold text-gray-700 dark:text-gray-300 min-w-[40px] text-right">{slideDuration}s</span>
            </div>
                  ...
                  <button key={s.value} onClick={() => setSpeed(s.value)}
                    className={`flex-1 px-3 py-2 rounded-lg text-center text-[12px] transition-all cursor-pointer ${
                      speed === s.value
                        ...
                        : "bg-gray-50 dark:bg-gray-800/30 text-gray-500 border border-gray-100 dark:border-gray-800 hover:border-gray-200"
                    }`}>
          ...
          {isConverting ? (
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-[12px] font-medium text-gray-600 dark:text-gray-400">Converting...</span>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={onClose}
                  className="px-4 py-2 rounded-xl text-[13px] font-medium text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors cursor-pointer">
                  Cancel
                </button>
