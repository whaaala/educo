...
export default function SlideMenuBar({ onAction }: SlideMenuBarProps) {
  ...
  const fileMenu: EditorMenuItem[] = [
    ...
    { label: "Share", icon: Share2, submenu: [
      ...
      { label: "Publish to web", icon: ExternalLink, onClick: act("file:publish") },
    ]},
    ...
    { label: "Download", icon: Download, submenu: [
      { label: "Microsoft PowerPoint (.pptx)", icon: FileText, onClick: act("file:downloadPptx") },
      { label: "ODP document (.odp)", icon: FileText, onClick: act("file:downloadOdp") },
      { label: "PDF document (.pdf)", icon: FileText, onClick: act("file:downloadPdf") },
      { label: "Plain text (.txt)", icon: FileText, onClick: act("file:downloadTxt") },
      { label: "JPEG image (current slide)", icon: Image, onClick: act("file:downloadJpeg") },
      { label: "PNG image (current slide)", icon: Image, onClick: act("file:downloadPng") },
      { label: "SVG (.svg, current slide)", icon: Image, onClick: act("file:downloadSvg") },
    ]},
    { label: "Convert to video", icon: Video, submenu: [
      { label: "Entire presentation", icon: Video, onClick: act("file:videoAll") },
      { label: "Selected slides", icon: Video, onClick: act("file:videoSelected") },
    ]},
    D,
