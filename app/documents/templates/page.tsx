    ...
    <MainLayout>
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-[20px] font-bold text-gray-900 dark:text-white">Template Gallery</h1>
            <p className="text-[13px] text-gray-500 dark:text-gray-400">{DOC_TEMPLATES.length} templates across {TEMPLATE_CATEGORIES.length} categories</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-[13px] text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all"
            />
          </div>
          <div className="flex gap-2 flex-wrap">
            <button
              onClick={() => { setActiveCategory(null); setSearchQuery(""); }}
              className={`px-3 py-1.5 rounded-lg text-[12px] font-medium transition-colors cursor-pointer ${!activeCategory && !searchQuery ? "bg-blue-500 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700"}`}
            >
              All
            </button>
            {TEMPLATE_CATEGORIES.map(cat => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-[12px] font-medium transition-colors cursor-pointer ${activeCategory === cat ? "text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700"}`}
        ...
        {!searchQuery && !activeCategory && (
          <div className="mb-6">
            <button
              ...
              className="w-full sm:w-auto flex items-center gap-3 px-5 py-4 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 hover:border-blue-400 hover:shadow-md transition-all cursor-pointer"
            >
              <div className="w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                <Plus className="w-5 h-5 text-gray-400" />
              </div>
              <div className="text-left">
                <p className="text-[14px] font-semibold text-gray-800 dark:text-gray-200">Blank document</p>
            ...
            return (
              <div key={cat} className="mb-8">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="text-[14px] font-bold text-gray-800 dark:text-gray-200">{cat}</h2>
                  <span className="text-[11px] text-gray-400 dark:text-gray-500 ml-1">{catTemplates.length} templates</span>
                </div>
  ...
  return (
    <button onClick={onClick} className="group cursor-pointer text-left w-full">
      ...
      <div className="mt-2 px-0.5">
        <p className="text-[13px] font-semibold text-gray-800 dark:text-gray-200 truncate">{template.label}</p>
        <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5">{template.category}</p>
      </div>
    </button>
  );
}
