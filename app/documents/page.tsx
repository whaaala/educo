      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="mb-6">
          <div className="relative max-w-[720px] mx-auto">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 rounded-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-[14px] text-gray-800 dark:text-gray-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all"
            />
          </div>
        </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-[13px] font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Recent documents</h2>
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                ...
                {sortOpen && (
                  <div className="absolute right-0 top-full mt-1 w-[180px] bg-white dark:bg-gray-900 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 z-50 py-1">
                    {([["lastOpened", "Last opened by me"], ["lastModified", "Last modified"], ["title", "Title"]] as const).map(([key, label]) => (
                      <button key={key} onClick={() => { setSortBy(key); setSortOpen(false); }}
                        className={`w-full px-3 py-2 text-left text-[12px] transition-colors cursor-pointer ${sortBy === key ? "text-blue-600 bg-blue-50 dark:bg-blue-900/20 font-medium" : "text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800"}`}>
                        {label}
                      </button>
                    ))}
                  </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 transition-opacity duration-300">
              {filteredDocs.map(doc => (
                <div key={doc.id} className="group relative cursor-pointer" onClick={() => handleOpenDoc(doc.id)}>
                  ...
                  <div className="aspect-[3/4] rounded-xl bg-white dark:bg-gray-900 border border-gray-200/60 dark:border-gray-700/50 shadow-[0_1px_3px_rgba(0,0,0,0.06)] dark:shadow-[0_1px_3px_rgba(0,0,0,0.2)] transition-shadow duration-300 group-hover:shadow-[0_8px_25px_rgba(0,0,0,0.1)] relative">
                    {/* Clip container */}
                    <div className="absolute inset-0 rounded-xl overflow-hidden">
                      ...
                      {doc.html ? (
                        <div className="w-[420px] origin-top-left pointer-events-none" style={{ transform: "scale(0.38)" }}>
                          <div
                            className="px-7 pt-5 pb-8 text-[13px] leading-[1.6] text-gray-800 dark:text-gray-200 [&_h1]:text-[22px] [&_h1]:font-bold [&_h1]:mb-2 [&_h1]:leading-tight [&_h2]:text-[16px] [&_h2]:font-bold [&_h2]:mt-4 [&_h2]:mb-1 [&_h3]:text-[14px] [&_h3]:font-semibold [&_h3]:mt-3 [&_h3]:mb-1 [&_p]:my-1 [&_ul]:pl-5 [&_ul]:my-1 [&_ul]:list-disc [&_li]:my-0.5 [&_hr]:my-3 [&_hr]:border-gray-200 [&_strong]:font-semibold"
                    ...
                    <div className="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                      <button onClick={e => { e.stopPropagation(); toggleStar(doc.id); }}
                        className={`p-1.5 rounded-full shadow-sm ${doc.starred ? "text-amber-400 bg-white dark:bg-gray-800" : "text-gray-400 hover:text-amber-400 bg-white/90 dark:bg-gray-800/90"} backdrop-blur-sm cursor-pointer transition-colors`}>
                    ...
                    {/* Context menu */}
                    {menuOpenId === doc.id && (
                      <div className="absolute top-10 right-2 w-[160px] bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl rounded-xl shadow-xl border border-gray-200/60 dark:border-gray-700/60 z-50 py-1"
                        onClick={e => e.stopPropagation()}>
                        <button className="w-full px-3 py-2 text-left text-[12px] text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-2 cursor-pointer transition-colors"
                  ...
                  <div className="mt-2.5 px-0.5">
                    <p className="text-[14px] font-semibold text-gray-800 dark:text-gray-200 truncate leading-tight">{doc.title}</p>
                    <div className="flex items-center gap-1.5 mt-1">
                      <FileText className="w-3 h-3 text-blue-500 flex-shrink-0" />
                      <span className="text-[11px] text-gray-400 dark:text-gray-500">{timeAgo(doc.updatedAt)}</span>
          ...
          {/* ── List View ── */}
          {viewMode === "list" && (
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 overflow-hidden transition-opacity duration-300">
              {/* Header */}
              <div className="grid grid-cols-[40%_20%_30%_10%] px-4 py-2 border-b border-gray-100 dark:border-gray-800 text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                <span>Name</span>
                <span>Owner</span>
              ...
              {filteredDocs.map(doc => (
                <div key={doc.id}
                  ...
                  className="grid grid-cols-[40%_20%_30%_10%] items-center px-4 h-[52px] border-b border-gray-50 dark:border-gray-800/50 last:border-b-0 hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-colors cursor-pointer group">
                  <div className="flex items-center gap-3 min-w-0">
                    <FileText className="w-4 h-4 text-blue-500 flex-shrink-0" />
                    <span className="text-[13px] text-gray-800 dark:text-gray-200 truncate">{doc.title}</span>
                    {doc.starred && <Star className="w-3 h-3 text-amber-400 fill-current flex-shrink-0" />}
                  </div>
                  <span className="text-[12px] text-gray-500 dark:text-gray-400">{doc.owner}</span>
                  <span className="text-[12px] text-gray-500 dark:text-gray-400">{timeAgo(doc.updatedAt)}</span>
                  <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button onClick={e => { e.stopPropagation(); setMenuOpenId(menuOpenId === doc.id ? null : doc.id); }}
                      ...
                      {menuOpenId === doc.id && (
                        <div className="absolute top-full right-0 mt-1 w-[160px] bg-white dark:bg-gray-900 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 z-50 py-1"
                          onClick={e => e.stopPropagation()}>
                          <button className="w-full px-3 py-1.5 text-left text-[12px] text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-2 cursor-pointer"
                            onClick={() => { handleOpenDoc(doc.id); setMenuOpenId(null); }}>
                            <ExternalLink className="w-3.5 h-3.5" /> Open
                          </button>
                          <button className="w-full px-3 py-1.5 text-left text-[12px] text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-2 cursor-pointer"
...
function TemplatePreviewCard({ template, onClick }: { template: typeof DOC_TEMPLATES[number]; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className="flex-shrink-0 w-[150px] group cursor-pointer text-left">
      <div className="aspect-[3/4] rounded-xl bg-white dark:bg-gray-900 relative transition-shadow duration-300 hover:shadow-xl"
