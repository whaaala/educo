...
export default function RootLayout({
  ...
  return (
    <html lang="en" className="h-full" suppressHydrationWarning>
      <head>
        {/* The website builder's whole font library — the school picks a family at RUNTIME, so the set cannot
            be enumerated at build time the way next/font requires. One stylesheet link is the right shape here.
            (The EXPORTED site does not use this: it embeds only the families in use, self-hosted, so a
            visitor's IP never reaches Google. See lib/educo-ui/font-embed.ts.) */}
        {/* eslint-disable-next-line @next/next/no-page-custom-font -- see above: a runtime-chosen font library. */}
        <link
      ...
      <body
        ...
        className={`${geistSans.variable} ${geistMono.variable} antialiased h-full bg-canvas transition-colors duration-300`}
      >
