...
## ARCHITECTURE (v9, CURRENT): RECURSIVE stack-of-rows everywhere
`normalizeRowBands` is now **recursive**: EVERY content container (page root, section, block — a non-`rowBand` container) is a vertical STACK whose direct children are all ROW BANDS; each row band is a **nowrap** row of items that **shrink to fit** (flex `0 1 X%` + `clampRowWidths` scales widths ≤100%). A bare item dropped directly under a content container is wrapped in its OWN full-width row → so **dragging an item DOWN makes a NEW row** (a second row appears only by dragging, never auto-wrap). Empty rows are pruned. **User margins are RESPECTED — never stripped** (`clampRowWidths` scales width only). Sections/blocks are `direction:column` content containers (`makeSection`/`makeBlock` in page.tsx), `clip:true`. `addChildSection` adds a block into the section's FIRST row (beside, all shrink). Resize of a row section = **pure DIVIDER** (grows this, shrinks the adjacent neighbour, keeps them packed, touches NO margins); an OUTER edge grows/shrinks into the row's leftover (capped ≤100%). Cross-axis (column-parent width) and height resize still use the page-bounded margin anchor ([[feedback_edge_anchored_resize]]). KEY=`educo_box_demo_v9`. Per-block backgrounds/images/overlays via Inspector (any block). Undo/redo (Ctrl+Z/Y). Drag: nearest-slot arrange within a parent (drop anywhere) + rAF-batched floating ghost; leaving the parent reparents. See [[project_box_builder]] for the website-builder vision.

## OLD ARCHITECTURE (v7): page = STACK of ROW BANDS
