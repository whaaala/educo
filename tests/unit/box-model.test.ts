...
import {
  ...
  containerStyle, childStyle, paddingCSS, marginCSS, sizeToCSS, flexForWidth, fillMainAxis, newBoxId,
  type BoxNode,
} from "@/lib/box-model";
...
describe("box-model — per-side spacing", () => {
  it("paddingCSS uses the general padding for every side by default", () => {
    expect(paddingCSS(createContainer("column", { padding: 20 }))).toEqual({ paddingTop: 20, paddingRight: 20, paddingBottom: 20, paddingLeft: 20 });
  });
  ...
  it("paddingCSS lets a per-side value override just that side", () => {
    ...
    expect(s).toEqual({ paddingTop: 60, paddingRight: 20, paddingBottom: 20, paddingLeft: 4 });
  });

  it("marginCSS falls back to the general margin and honours per-side overrides", () => {
    expect(marginCSS(createContainer("column", { margin: 12 }))).toEqual({ marginTop: 12, marginRight: 12, marginBottom: 12, marginLeft: 12 });
    expect(marginCSS(createContainer("column", { margin: 12, marginBottom: 40 })).marginBottom).toBe(40);
    expect(marginCSS(createContainer("column", {})).marginTop).toBeUndefined(); // no margin at all
  });

  it("containerStyle applies per-side padding", () => {
    const s = containerStyle(createContainer("column", { padding: 10, paddingTop: 50 }));
    expect(s.paddingTop).toBe(50);
    expect(s.paddingBottom).toBe(10);
  });
});
